<template>
    <v-card outlined>
        <v-card-title>
            Target # {{item._links.self.href.split("/")[item._links.self.href.split("/").length - 1]}}
        </v-card-title>
        ...
        <v-card-text>
            ...
            <div>
                <String label="ModelRequestId" v-model="item.modelRequestId" :editMode="editMode" @change="change" />
            </div>
        </v-card-text>
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            var temp = await axios.get(axios.fixUrl('/targets/' + params.id))
            if(temp.data) {
                me.item = temp.data
            }
        },
