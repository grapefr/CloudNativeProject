<template>
    <v-card style="width:450px; height:100%;" outlined>
        ...
        <v-card-title v-if="value._links">
            Target # {{decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1])}}
        </v-card-title >
        <v-card-title v-else>
            Target
        </v-card-title >        
        ...
        <v-card-text>
            ...
            <String label="ModelRequestId" v-model="value.modelRequestId" :editMode="editMode" :inputUI="''"/>
        </v-card-text>
        ...
        methods: {
            ...
            async save(){
                try {
                    ...
                    if(!this.offline) {
                        if(this.isNew) {
                            temp = await axios.post(axios.fixUrl('/targets'), this.value)
                        } else {
